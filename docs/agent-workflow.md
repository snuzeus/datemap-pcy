...
| `docs/qa-checklist.md` | 작업 완료 후 사용자에게 제공할 QA 체크리스트 양식과 이슈별 검증 항목. |
...
② Claude에게 요청 (패턴 A~D 중 선택)
        ↓
③ Claude가 구현 → 변경 사항 설명 + `docs/qa-checklist.md` 양식의 QA 체크리스트 제공
        ↓
...
작업 완료 후 사용자에게 검증 항목을 전달할 때는 반드시 `docs/qa-checklist.md`의 구조를 따른다.
완료한 이슈 이름을 기준으로 해당 Feature 섹션을 찾아, 사용자가 직접 실행할 수 있는 체크박스 형식으로 정리한다.
새 이슈를 완료했는데 `docs/qa-checklist.md`에 해당 항목이 없으면, 같은 문서의 기존 섹션 형식을 맞춰 QA 항목을 추가한 뒤 안내한다.
