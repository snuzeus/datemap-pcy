...
- [x] **[ISSUE] Supabase DB 스키마 설계 및 마이그레이션** `infra` (High)
...
- [x] **[ISSUE] 지역 핫도 스코어 산출 로직 구현** `data` (High)
  - 지역 기본 관심도 + 혼잡도 가중치 합산 → `region_hot_score` 계산
...
- [x] **[ISSUE] Supabase 캐싱 전략 — API fallback 처리** `data`
...
- [x] **[ISSUE] 카카오맵 JavaScript API 연동 — 지도 초기화** `feature` (High)
...
- [x] **[ISSUE] 필터 태그 상수 정의** `feature` (High)
  - `constants/filterTags.ts` — 카테고리 7개, 무드 7개 고정값
- [x] **[ISSUE] 필터 바 컴포넌트 구현** `ui` (High)
  - `components/FilterBar.tsx` — 카테고리 탭 + 무드 멀티셀렉트 칩
- [x] **[ISSUE] 필터 상태 관리 — URL 쿼리 파라미터 동기화** `feature`
  - `useSearchParams` 기반 필터 상태 유지 (새로고침 시 복원)
- [x] **[ISSUE] 필터 적용 시 장소 목록 실시간 업데이트** `feature` (High)
...
- [x] **[ISSUE] 장소 상세 페이지 라우팅 구성** `feature` (High)
  - `app/place/[placeId]/page.tsx` 동적 라우트
- [x] **[ISSUE] 장소 상세 정보 컴포넌트 구현** `ui`
  - 이름, 주소, 카테고리, 무드 태그, 리뷰 요약, 카카오맵 단독 마커
- [x] **[ISSUE] 리뷰 섹션 컴포넌트 구현** `ui`
  - `components/ReviewList.tsx` — 카카오맵 리뷰 데이터 렌더링
- [x] **[ISSUE] 저장 버튼 컴포넌트 구현** `ui` (High)
...
- [x] **[ISSUE] Supabase Auth 설정 — 소셜 로그인 (카카오/구글)** `feature` (High)
  - `app/api/auth/route.ts`, 로그인 모달 컴포넌트
- [x] **[ISSUE] 장소 저장/해제 API Route 구현** `feature` (High)
  - `app/api/saved-places/route.ts` — POST / DELETE
- [x] **[ISSUE] 저장된 장소 목록 페이지 구현** `feature` `ui`
  - `app/saved/page.tsx` — 저장 장소 그리드, 코스 생성 버튼
- [x] **[ISSUE] 코스 저장 API Route 구현** `feature` (High)
  - `app/api/courses/route.ts` — 장소 배열 → 코스 생성
- [x] **[ISSUE] 공유 링크 생성 및 공유 페이지 구현** `feature`
...
- [x] **[ISSUE] 동선 순서 편집 UI 구현** `ui`
  - `components/CourseEditor.tsx` — 드래그 앤 드롭 장소 순서 정렬
- [x] **[ISSUE] 카카오맵 경로 폴리라인 렌더링** `feature`
  - 저장 장소 간 이동 경로 지도 위에 표시
- [x] **[ISSUE] 코스 요약 카드 컴포넌트 구현** `ui`
...
- [x] **[ISSUE] Vercel 프로젝트 연결 및 환경변수 설정** `deploy` (High)
