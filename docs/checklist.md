...
- [ ] **[ISSUE] 홈 화면 레이아웃 구성** `ui` (High)
  - `app/page.tsx` — 지역 랭킹 카드 그리드 레이아웃
- [ ] **[ISSUE] 지역 랭킹 카드 컴포넌트 구현** `ui` (High)
  - `components/RegionCard.tsx` — 지역명, 핫도 지표, 대표 이미지
- [ ] **[ISSUE] 지역 랭킹 데이터 패칭 훅 구현** `feature` (High)
  - `hooks/useRegionRanking.ts` — Supabase에서 핫도 순위 fetch
- [ ] **[ISSUE] 핫도 순위 표시 UI — 실시간 배지 및 트렌드 인디케이터** `ui`
...
- [ ] **[ISSUE] 장소 목록 페이지 라우팅 구성** `feature` (High)
  - `app/region/[regionId]/page.tsx` 동적 라우트
- [ ] **[ISSUE] 장소 카드 컴포넌트 구현** `ui` (High)
  - `components/PlaceCard.tsx` — 장소명, 카테고리, 무드 태그, 썸네일
- [ ] **[ISSUE] 장소 목록 데이터 패칭 훅 구현** `feature` (High)
...
| Feature | 이슈 수 | 마일스톤 |
|---------|---------|----------|
| 1. 프로젝트 초기 세팅 | 6개 | Week 1 |
| 2. 데이터 파이프라인 | 6개 | Week 1 |
