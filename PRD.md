...
| 데이터 | 출처 | 활용 목적 |
|--------|------|-----------|
| 검색어 트렌드 | 네이버 데이터랩 API | 지역·장소별 검색량 증감으로 '요즘 핫한 지역' 순위 산출 |
...
### 데이터 수집 방식
- Phase 1: 외부 공개 API (네이버 데이터랩, 서울 도시데이터) + 크롤링/파싱 (네이버 플레이스, 카카오맵 리뷰)
...
| 데이터 수집 | 네이버 데이터랩 API + 서울 실시간 도시데이터 API (서버사이드 cron) | 지역 핫도 랭킹 산출 원천 데이터 |
...
| 주차 | 기간 | 주요 내용 |
|------|------|-----------|
| Week 1 | 05/27 ~ 05/31 | DB 스키마 설계, 외부 API 연동 (네이버 데이터랩 + 서울 도시데이터), 핫플 지역 랭킹 산출 로직 |
...
| 네이버 데이터랩 API 호출량 제한 | 중간 | 배치 수집(1일 1회 cron)으로 호출 최소화 |
| 3주 내 동선 묶기까지 구현 불가 | 낮음 | 동선 기능은 Post-MVP으로 분리, 핫플 탐색 + 저장/공유로 MVP 완성 |

### 핵심 가정
- 네이버 데이터랩 검색량 트렌드가 실제 지역 방문 핫도와 유의미하게 상관관계가 있다
